...
lipid <- readRDS("../data/untargeted_singletp_fin4_all(single_istd).rds")
```
...
all.lipid <- colSums(peak.rel)
target.sp.conc <- peak.rel[target.sp, ]
...
ggplot(df)+
        ...
        labs(title = peak$fdata[target.sp, "name"], x = "", y = "Normalized Peak Height") +
        theme_cynthia_bw()
...
ggplot(df, aes(x="", y=target.sp)) +
        ...
        labs(x= "", y = "Normalized Peak Height", title = peak$fdata[target.sp, "name"]) +
        theme_cynthia_bw()
```
